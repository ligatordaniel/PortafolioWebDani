...
**Regenerate the crawlable English page after content/translation changes:**

```bash
node assets/docs/generar_pagina_en.mjs
```

**Regenerate both CV PDFs after editing their HTML sources:**

```bash
bash assets/docs/generar_pdf.sh
```
...
- `index.html` — canonical Spanish page markup, SVG symbols, content sections,
  metadata, JSON-LD, modal markup, and links to external CSS/JS.
- `en/index.html` — generated, crawlable English version. Do not edit it by
  hand; regenerate it with `assets/docs/generar_pagina_en.mjs`.
...
- `assets/js/main.js` — reveal animations, day/night mode toggle, language
  navigation, translation maps, accessible modal behavior, and CV download
  helper.
...
- `llms.txt`, `sitemap.xml`, and `robots.txt` — machine-readable discovery
  files for AI systems and search crawlers.
...
- If adding or renaming translated text, update both language maps and the
  matching `data-i18n` keys, then regenerate `en/index.html`.
- Keep claims, dates, project stacks, `llms.txt`, and both CV language versions
  synchronized. Do not add performance metrics that cannot be verified.
